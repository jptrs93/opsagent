...
test('bootstrap primary, enroll worker, and create Nix Docker deployment', async ({page}) => {
  ...
  await expectOpenDeployAgentVersion(page, {machine: 'primary', version: process.env.OPD_INSTALL_VERSION || 'v0.0.256'});
  ...
  await upgradeOpenDeployAgents(page, {version: process.env.OPD_UPGRADE_VERSION || 'v0.0.256'});
