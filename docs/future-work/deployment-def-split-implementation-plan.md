...
Status: complete. Phase 1 shipped in v0.0.553 (envelope+def split, column-backed rows, bridge fields live); phase 2 landed after the v0.0.553 rollout reached every cluster: the bridge flat fields are deleted with their numbers reserved, the def→flat mirror and flat→def fold are gone, and the v0.0.553 migrations were stripped per the migrations.sql convention. The optional clean blob re-encode/renumber was not done — `DeploymentDef` keeps its frozen numbers (2/8/10/11) so historical event rows keep decoding, which costs nothing. Since v0.0.611 placement and desired running state live in `Deployment.scheduling` (`Scheduling { running, dedicated_nodes { nodes } }`, its own `scheduling_version` facet). The one-release bridge (`node_id`, tag 2, mirrored on read for workers that predated scheduling, and `ContainerSpec.running`, tag 4, read by the one-time blob lift) was removed after the v0.0.611 rollout reached every cluster: both tags are reserved, the mirror and the lift are gone, and the v0.0.611 statements were stripped from migrations.sql per its convention.
