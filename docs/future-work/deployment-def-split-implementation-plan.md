...
Status: complete. Phase 1 shipped in v0.0.553 (envelope+def split, column-backed rows, bridge fields live); phase 2 landed after the v0.0.553 rollout reached every cluster: the bridge flat fields are deleted with their numbers reserved, the def→flat mirror and flat→def fold are gone, and the v0.0.553 migrations were stripped per the migrations.sql convention. The optional clean blob re-encode/renumber was not done — `DeploymentDef` keeps its frozen numbers (2/8/10/11) so historical event rows keep decoding, which costs nothing. Since v0.0.611 placement and desired running state live in `Deployment.scheduling` (`Scheduling { running, dedicated_nodes { nodes } }`, its own `scheduling_version` facet); `node_id` (tag 2) is mirrored from `scheduling.dedicated_nodes.nodes[0]` when a row is read, for workers that predate scheduling, and `ContainerSpec.running` (tag 4) is read only by the one-time blob lift in `pq/migrate_scheduling.go`. Both tags are reserved, and the mirror and lift removed, once every cluster has rolled forward — the same two-phase pattern as the split itself.
