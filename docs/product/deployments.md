...
volume mounts, and asset mounts do not require the custom host-mount permission.

Every update checks the existing and proposed configuration. Changing a version,
command, environment, or running state requires the relevant permission even
when the host settings are unchanged. Removing host access also requires it.
Moving a deployment to another space requires the relevant host permissions in
both spaces, plus the normal destination create permission. Forks, including
forks from deleted deployments, follow the same checks as any other create.

Human cluster admins have both permissions across all spaces. Space admins and
agent sessions do not inherit them from the built-in roles. Cluster admins can
grant them for selected spaces or existing deployments through Users & roles.
Agent access requires an explicitly delegable grant. Existing workloads keep
running when the built-in policy updates; subsequent create/update requests
use the new permissions.

Custom paths are a privileged escape hatch. Validation requires clean absolute
paths, trims surrounding whitespace, and rejects protected host directories,
their descendants, and their ancestors. For example, protecting
`/var/lib/opendeploy` also prohibits mounting `/var/lib`, `/var`, or `/`.
The denylist applies to read-only and writable mounts alike.

The denylist checks path strings on the primary. It does not resolve symlinks
or inspect the filesystem on the scheduled node. Cluster administrators are
responsible for ensuring that allowed paths cannot expose protected material
through symlinks, bind-mount aliases, or other alternate filesystem paths.
They must also control the code and configuration consumed by workloads with
host access. A grant scoped to one space still conveys trust involving the
underlying node; it does not confine host access to that space.
