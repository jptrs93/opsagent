...
<body><a class="skip" href="#findings">Skip to findings</a><div class="topbar"><div class="shell"><div class="brand">OPENDEPLOY / SECURITY REVIEW</div><span>Re-review of the 11 September audit · 13 September 2026</span></div></div>
<header class="hero shell"><div class="eyebrow">Security re-review / September 2026</div><h1>The host boundary is largely closed. The tenant boundary is not.</h1><p class="lede">Since the 11 September audit, Nix builds moved into a locked-down container, host mounts and host networking became cluster-admin-only, and the runtime and HTTP/2 dependencies were updated. What remains sits mostly between tenants and at the edges: hostnames are not bound to spaces, workloads have no resource or syscall limits, host ports and request paths are not arbitrated, and the primary's public listeners have no deadlines.</p><div class="meta"><span>Reviewed 13 September 2026 · Australia/Sydney</span><span>Commit <code>{COMMIT[:12]}</code></span><span>Baseline <code>{BASELINE[:12]}</code></span><span>Source review, rerun reproductions, dependency scans</span></div><div class="scorecard">{stats}<div class="stat-note"><strong>{len(FINDINGS)} open findings</strong>{sum(1 for f in FINDINGS if f['origin']=='audit')} carried from the 11 September audit, {sum(1 for f in FINDINGS if f['origin']=='review')} from the 12 September assessment that followed it, {sum(1 for f in FINDINGS if f['origin']=='new')} new. Every item was re-verified against commit {COMMIT[:7]}.</div></div></header>
...
<section id="closed"><div class="eyebrow">Closed since 11 September</div><h2>What the intervening commits fixed.</h2><p class="small">Each closure was confirmed by reading the current code and, where the audit had a reproduction, by that reproduction no longer passing. Fixed code does not retire credentials or state exposed before the fix.</p><div class="table-wrap"><table><thead><tr><th>ID</th><th>Finding</th><th>Was</th><th>How it closed</th><th>Evidence</th></tr></thead><tbody>{closed_rows}</tbody></table></div>
...
</main><footer class="shell">Prepared by Claude Code from the local OpenDeploy repository. Review date: 13 September 2026. Findings describe commit {COMMIT[:12]} and stated assumptions; no remediation was performed as part of this review. This HTML uses no external scripts, fonts or analytics.</footer><script>{JS}</script></body></html>'''

OUT.write_text(document)
(EVIDENCE / "findings.json").write_text(json.dumps({"commit": COMMIT, "baseline": BASELINE, "date": "2026-09-13", "findings": FINDINGS, "closed": CLOSED, "accepted": ACCEPTED}, indent=2) + "\n")
