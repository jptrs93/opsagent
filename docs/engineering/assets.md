...
- Asset content and version metadata are immutable. Saving an existing key appends the next integer version; `location` changes only after a durable storage transition.
