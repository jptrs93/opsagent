...
- **Cross-space moves are expressible but not yet supported.** Both move requests carry a `space_id`: `0` keeps the row where it is, and naming a different space is refused with `asset_space_move_unsupported` (`MoveAssetSpace` / `MoveDirectorySpace` are the gates; assets carry space-scoped mounts and references, so the move needs coordinated handling). The field exists so callers can state the intent and get that answer — without it, `asset_directory_id: 0` against another space would silently land the row at *its own* space's root. The gate therefore runs before any reparenting, and the explorer's drag-and-drop offers cross-space drops and surfaces the server's refusal. When the move lands, only the two store gates change.
