...
- The wire `location` is derived, not stored: empty for inline versions, `local://<store-id>` when the local side is durable, `s3://<store-id>` when the S3 side is. The wire `sha256` is the content hash, blank while a migrated row's content has not been hashed yet.
...
The 2026-08 content split moved blobs and locations off `asset_versions` into `asset_store`. The SQL migration seeds one store row per version row, preserving the version row id as the store row's `id` — local files were already named `<version-row-id>` and S3 keys `<s3-path>/<version-row-id>`, so the id-derived naming resolves every existing copy with **zero renames or S3 copies**, and older database restore points keep referencing valid object keys. New rows use uuidv7 ids, which cannot collide with the numeric legacy ids. In-flight `pending://` rows (a crash mid-upload immediately before the upgrade) are dropped by the migration along with any asset left versionless.

Migrated rows get a `legacy:<id>` placeholder sha (unique, so the version link works immediately, and blanked on the wire). A background converter in the reconciler loop hashes each such row's content — inline and local rows cheaply, S3 rows via one download — and atomically repoints its version links to the real sha. When the real sha already exists (duplicate content), the versions repoint to the existing row and the duplicate row and its local file are reclaimed; its S3 object, if any, is retained. Location naming stays derivable because changing the effective S3 configuration is rejected while anything is S3-backed or staging, so the current settings always describe where every S3 copy lives.
