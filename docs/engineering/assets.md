...
- Folder tree API: `/v1/asset-directories/{list,create,move,rename,delete}` manages `asset_directories`; delete is rejected unless the folder is empty, and moving a folder inside its own subtree is a cycle error. `/v1/assets/move` (`{asset_id, asset_directory_id}`) relocates an asset within its space — version rows and ids are untouched, so pinned deployment references survive; cross-space targets are rejected. The state stream carries `asset_directories_snapshot`/`asset_directory_update` (deletes arrive with `deleted` set) and `GET /v1/global-state` includes `asset_directories`.
