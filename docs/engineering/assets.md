...
- Storage tables: `assets` holds the stable identity — `(space_id, asset_directory_id, key)` plus `created_at`/`author`; `asset_versions` holds the immutable version rows with unique `(asset_id, version)`, each carrying `size_bytes` and the `sha256` of its content; `asset_store` holds the content itself, keyed by a uuidv7 `id` with a unique `sha256`, an `inline_blob` for small content, and `local_status`/`remote_status` flags saying which storage side holds a durable copy; `asset_directories` holds the per-space folder tree (`parent_id`, `0` = the implicit root). `author` is the acting user id, `0` for migrated or system rows.
