...
- Storage tables: `assets` holds the stable identity — `(asset_directory_id, key)` plus `created_at` and `deleted_at` (`0` = live; deletion is soft, so version rows and content survive but every read excludes the asset); `asset_spaces` is the append-only log of space assignments (`asset_id, author, created_at, space_id`) — creation writes the first row, each cross-space move appends one, and the newest row is the asset's current space; `asset_versions` holds the immutable version rows with unique `(asset_id, version)`, each carrying `size_bytes` and the `sha256` of its content; `asset_store` holds the content itself, keyed by a uuidv7 `id` with a unique `sha256`, an `inline_blob` for small content, and `local_status`/`remote_status` flags saying which storage side holds a durable copy; `asset_directories` holds the per-space folder tree (`parent_id`, `0` = the implicit root). Version rows carry `author`, the acting user id, `0` for migrated or system rows.
...
- Assets are listed via `/v1/assets/list`, one `Asset` per asset: `id` and `deleted_at` at the root, the filesystem identity in `fs` (`key`, `directory_id`), the append-only space log in `space_versions` (newest first — `[0]` is the current space, each row stamped with author and `global_seq`), and the content index in `content_versions` (newest first — `[0]` is the latest, each row carrying the version row id, number, creation info, author, size, sha256, and `global_seq`). The content version row ids are what deployment specs pin, so that index is what usage matching and version-pinned references join against. Version rows only exist once their content is durable, so every asset with a version is listable. Content bytes never travel inline: they are streamed via `GET /v1/assets/content?content_version_id=N`, the web equivalent of the workers' cluster asset endpoint.
