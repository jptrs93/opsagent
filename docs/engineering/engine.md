...
Inputs run before the image stage so a cheap and commonly transient failure — a secret the primary has not distributed yet — fails before committing to a build that can take minutes. The two stages are otherwise independent. `prepare.StatusUpdate` is the only way to publish a transition. Nix builds are serialized per repository, because each repository store has one writer, and capped node-wide at two concurrent build containers by the network manager's build attachment limit; image pulls and the two internal release preparations are allowed to run independently.
...
`nixdocker.Preparer` derives a node-local image ref from a cache schema version, the Nix repository/flake/target, the node platform, and `ContainerSpec.Version`. The ref is `opendeploy.local/nix-docker-build/v2/{sourceHash}:{commit}` and is intentionally independent of deployment ID and runtime-only config. The preparer reuses that image when it is already present and unpacked in containerd. Bumping the cache schema version creates a new image namespace and lazily invalidates images produced under older preparer semantics; `v2` is the nix2container contract.

A cache miss takes the repository's build slot (`nixstore.Manager.Acquire`, one build at a time per repository key, the hex SHA-256 of the trimmed repository URL that the Git manager also uses), asks `repo/git.Manager` to prepare a local checkout at the desired commit, and verifies the configured repository-relative `flake.nix` path is still a regular file in that checkout. `nixstore.Manager.Ensure` then makes sure the build image is present (the pinned `nixos/nix` digest, overridable with `OPENDEPLOY_NIX_BUILD_IMAGE`), seeds a template for that image digest once per node by copying the image's `/nix` out of a maintenance container, and creates the repository store from the template with `cp -al` for `store/` and a plain copy of `var/`. A store seeded from an older image digest, or one with an outstanding operator reset, is deleted and reseeded before the build. The store layout is `/var/lib/opendeploy-nix/{template/<digest>/nix, stores/<key>/{nix, tmp/<build>, store.json, verified-layers.json}, etc/nix.conf}`.

The build runs through `ctrd.Client.RunBuild`: the pinned image as container root, the store read-write at `/nix`, the checkout read-only at `/build/src` with its `.git` entry masked, the node's generated `nix.conf`, a per-build scratch directory at `/build/tmp` backing `TMPDIR` and `HOME`, and, when `OPENDEPLOY_NIX_BUILD_CA_BUNDLE` is set, that bundle mounted with `SSL_CERT_FILE`, `NIX_SSL_CERT_FILE` and `GIT_SSL_CAINFO` pointing at it. Nothing else from the agent environment is forwarded and the container receives no credentials. The command is `nix build --no-update-lock-file --no-link --print-out-paths -L path:/build/src?dir=<flake dir>[#<target>]`; a `path:` reference keeps `nix` from running Git against the checkout. The container joins a build network attachment created by the network manager under the reserved deployment identity in space 0: DNS through netproxy, egress to public addresses, and no reachability of the cluster prefix, peer node underlay addresses, link-local or loopback ranges, with at most two attachments per node. It runs under the runtime default seccomp profile and capability set with a memory limit, CPU quota and pids limit from `OPENDEPLOY_NIX_BUILD_MEMORY_MB`, `OPENDEPLOY_NIX_BUILD_CPUS` and `OPENDEPLOY_NIX_BUILD_PIDS` (defaults: three quarters of the node's memory with a 512 MiB floor, every CPU, 4096 pids), and an open-file limit (`RLIMIT_NOFILE`) from `OPENDEPLOY_NIX_BUILD_FILE_DESCRIPTORS` (default 4096) in place of `ctrd`'s 2048 default, which builders inherit as children of the container process. Stdout and stderr stream to the prepare log; an OOM kill recorded in the container cgroup is reported as a memory limit failure, and a `nix` failure whose log mentions an authentication prompt is reported as a flake input that needs credentials.

The last stdout line is the store path of the flake output, which must be a `nix2container.buildImage` image description: JSON naming the OCI config and every layer with its digest, size, diff id and store paths. `nix2container.Ingest` reads the description from the repository store, regenerates each layer tar from the store paths under that root with entry names rewritten to `/nix/store`, hashes it while writing it to the containerd content store, writes the config and manifest, tags the local ref and unpacks it. A layer is written from an existing blob only when its digest appears in the agent's own verified record for that store (`verified-layers.json`, keyed by the sorted path set); a digest claimed by the description that the agent has not computed is regenerated, and a mismatch fails the import. No file produced by a build is ever executed on the host.

After a build the store size is logged and, above `OPENDEPLOY_NIX_STORE_SIZE_CAP_MB` (default 15 GiB), `nix store gc` runs in a maintenance container. A background loop on every node deletes and reseeds stores older than `OPENDEPLOY_NIX_STORE_RESET_HOURS` (default 7 days) and stores with an operator reset outstanding, always under the repository's build slot. Operator resets are requested through `POST /v1/nix-store/reset` (cluster update authority), persisted on the primary in `nix_store_resets`, applied to the primary's own stores, and sent to every secondary at the head of its cluster session and on change; each node reseeds a store before its next build when the request is newer than the store's seed time. The store is a cache: the containerd image is the durable artifact and `--no-link` leaves no garbage-collection roots. Poisoning a store is contained to later builds of the same repository and bounded by the scheduled and operator resets; see `docs/future-work/containerized-nix-builds.md` for the stance.
...
Public deployments run through `containerRunner`. It creates one containerd container per run, named `opendeploy-{deploymentID}-{version}-{scheduledInstanceID}-{run}` (the prefix without the run is the placement's container family), wires stdout/stderr through a dedicated binary log consumer, waits for task exit, and respawns with exponential backoff on crashes. The consumer writes merged half-hourly UTC files under `{LogWALDir}/{deploymentID}/{YYYYMMDD_HHMM}_{version}_{run}.logbin`. Each binary record carries its timestamp, version, run number, and stdout/stderr stream. `version` is the top-level deployment version of the placement's pinned config, not the spec version in the container name: a placement runs exactly one deployment version and every top-level bump (spec change, stop and start, rename, space move) creates a new placement, so `(node, version, instance, run)` names one container lifetime even though each placement's runs restart at `1`. `run` starts at `1` and increments for each respawn within the placement. A rollover candidate has a newer deployment version, so it writes distinct files while it overlaps the old runner. Host-network deployments join the host network namespace. Virtual-network deployments get a pre-created Linux netns from `backend/lib/network`, and the containerd OCI spec joins that namespace.
...
- Reattach lists the family prefix rather than an exact id: the restart counter is only persisted after the task is created, so a crash in between leaves the running container one run ahead of the status. The newest run is adopted, its number is written back to the status, and older leftovers are removed. If nothing is running the runner starts fresh.
- Stop sends SIGTERM, waits up to 3 seconds, sends SIGKILL if needed, then deletes the task/container/snapshot.
- A lost exit stream is not an exit. containerd is restarted by package upgrades (needrestart after a libc update) and by bundled-binary upgrades, and its unit uses `KillMode=process` so shims and containers survive; only the gRPC wait stream breaks. When the stream yields an error the runner polls the task's status every second for up to a minute: still running means it re-issues the wait on the same handle and the container is never touched; stopped means the exit code is read from the retained task status; not found, or no answer within the window, is treated as an exit with unknown code and takes the normal crash path.
