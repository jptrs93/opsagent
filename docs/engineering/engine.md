...
- Each run gets its own cgroup, `/opendeploy/{deploymentID}-p{scheduledInstanceID}-v{version}-r{run}`, set as the OCI `cgroupsPath`. The runner registers the run with `lib/metrics` (node-level resource sampler) once the task is up and deregisters it in `deleteTask`; see `docs/future-work/container-metrics-implementation-plan.md`.
