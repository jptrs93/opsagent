...
- Before every start the runner removes any other container in its family (a crash between a task delete and the next create can leave one behind) and the legacy `opendeploy-{deploymentID}-v{version}` container an older agent may have created.
- Reattach lists the family prefix rather than an exact id: the restart counter is only persisted after the task is created, so a crash in between leaves the running container one run ahead of the status. The newest run is adopted, its number is written back to the status, older leftovers are removed, and the legacy name is tried when the family is empty. If nothing is running the runner starts fresh.
