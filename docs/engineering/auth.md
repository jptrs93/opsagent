...
Two builtin templates are seeded (and re-asserted) at startup: `cluster_admin` (everything) and `space_admin(spaces)` (everything in the bound spaces except `use_host_mounts` and `use_host_network`). Each then carries two delegable rules that together define what an agent session inherits:

- everything **except the `secret` entity type and the `view_logs`, `use_host_mounts`, and `use_host_network` verbs**, in every space except 0 (or the bound spaces) — cluster-level entities stay human-only by default, host access requires an explicit grant, and logs stay off-limits because a deployment can echo secret values into them, which would sidestep the secret rule below;
- `view` and `create` on `secret` in the same spaces — an agent can see which secrets exist, mint a credential, and wire it into a deployment, but cannot reveal, edit, or delete one. Wiring a secret into a deployment the agent controls does place the value in that workload; see [accepted positions](#accepted-positions) for what the reveal verb does and does not bound.

`use_host_mounts` and `use_host_network` are additional permissions on the `deployment` entity. A create requires the normal `create` permission plus the permissions implied by the proposed spec. Every update requires normal `view` and `update` access plus the host permissions implied by either the saved or proposed spec. This includes version changes, start/stop, full spec replacement, removing host access, and space moves. A space move also requires `create` in the destination and the relevant host permissions there, checked against the existing deployment ID. Custom host mounts are `container1_spec.runtime.mounts`; managed default volumes, cross-deployment volumes, and asset mounts do not require `use_host_mounts`. For proposed specs, explicit `HOST` networking requires `use_host_network`; unspecified networking defaults to virtual during validation. Saved specs follow the runtime: any non-virtual mode, including an old unspecified mode, requires `use_host_network` on update.

Human cluster admins receive both host permissions across all spaces. Space admins and agents receive neither from the builtins. Administrators may add scoped grants; an agent receives such a grant only when `delegation_allowed` is set. Existing global denies still take precedence. A scoped host permission limits where the caller may submit changes, but the exposed host resources can affect other spaces on that node. See [host access responsibilities](../product/deployments.md#host-access-permissions).

Builtin upgrades use `authz.Open` and `upsertBuiltinRuleTemplate`: changed definitions append a template update event under the existing ID before grants are loaded. Grants refer to that ID, so existing users adopt the new policy without grant rewrites. Identical definitions do not append events on subsequent starts. No custom-rule migration accompanies these permissions.
...
These are deliberate properties of the access model, recorded so that reviews classify them as documented authority (Informational under [security-severity.md](security-severity.md)) rather than as findings. A review that disagrees with one should propose changing the position here, not file the behaviour at a higher severity.

**AP-1 — Two permissions grant access to a secret value.** `secret : reveal` is the direct path and is a confidentiality boundary for it: without reveal, no API call returns a plaintext. It is not the only permission that gives access to the value. `deployment : create` or `deployment : update` in a space from which a secret can be bound (the deployment's own space and the global space, per `validateRefSpaces`) gives indirect access: the caller binds the secret into a workload they control and the workload reads its own environment. Deployment writes do not check any secret grant, so a principal has access to a secret's value if they hold reveal on it **or** deployment create or update in its space or, for a global-space secret, in any space. To keep a value from a principal, withhold all of those permissions; withholding reveal alone withholds only the direct path. This is intentional: requiring a secret grant on every deployment write would make deployment authorship depend on secret administration, and a space admin who can run arbitrary workloads in a space is trusted with what those workloads can bind. The asymmetry with `PostV1SecretsCreate`, where supplying a value requires reveal, is deliberate too: choosing a value is knowing it, whereas obtaining a bound value requires building a workload that exposes it.
...
- The Users page's rule explainer (`frontend/src/lib/authzExplain.js`) renders what a rule allows from three tables of the verb × entity pairs the handlers check: `APPLICABLE` (every pair), `SYSTEM_APPLICABLE` (the part checked in space 0: the cluster-level types, space creation, and the system deployments' view, update, delete, `view_logs` and both host permissions) and `SPACE_APPLICABLE` (the part checked in a real space). A handler that starts checking a new pair, or stops, must update them; `authzExplain.test.js` pins the partition. `view_logs` is a deployment permission only: a node's system log (`deployment_id = 0` on `/v1/deployments/log-query`) is the log of the opendeploy system deployment on that node, and `logQueryTargetNode` gates it on that deployment's `view_logs` in space 0, so nodes carry no log permission of their own.
