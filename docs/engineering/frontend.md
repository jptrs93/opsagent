...
- The deploy overlay fetches available versions on demand via `POST /v1/deployments/versions`, lets the user edit the deployment spec form, and submits a typed `DeploymentUpdateRequestV2` via `POST /v2/deployments/update`.
...
- `components/deploymentUiWidget.js` and `components/deploymentCodeWidget.js` are independent middle surfaces.
