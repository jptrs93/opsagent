...
- `components/deploymentSource.js` defines pure source keys, validation requests, response attestation, and local flake-path rules. `DeploymentCreationUpdate` owns independently sequenced repository, branch-commit, exact Nix, and image discovery state; only exact repository-wide commit and flake validation permits Running Nix submissions.
...
- `asset("name"[, { version = number }])`, `secret("name"[, { version = number }])`, and `config("name"[, { version = number }])` resolve omitted versions to the latest version in the selected space. Code mode always emits pinned versions for existing deployments; create mode may omit the options object for a latest-version reference.
- HCL deployment references are always explicitly space-qualified: `address("space", "deployment")` resolves a virtual-network address and `deployment("space", "deployment")` resolves another deployment's default volume.
