 ...
 * @property {ContainerPort[]} ports
 * @property {HostPort[]} hostPorts
 */
/**
...
/**
 * @param {ContainerPort} message
 * @param {Writer} writer
 */
export function writeContainerPort(message, writer) {
    if (message.name !== undefined && message.name !== null && message.name !== "") {
        writer.uint32(tag(1, WIRE.LDELIM)).string(message.name);
    }
 ...
 * @returns {Uint8Array}
 */
export function encodeContainerPort(message) {
    const writer = Writer.create();
    writeContainerPort(message, writer);
    return writer.finish();
}
 ...
 */
function decodeContainerPortMessage(reader, length) {
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = {name: "", port: 0 };
    while (reader.pos < end) {
        const tag = reader.uint32();
        switch (tag >>> 3) {
            case 1: {
 ...
 * @returns {ContainerPort}
 */
export function decodeContainerPort(buffer) {
    const reader = Reader.create(new Uint8Array(buffer));
    return decodeContainerPortMessage(reader);
}



/**
 * @param {HostPort} message
 * @param {Writer} writer
 */
export function writeHostPort(message, writer) {
    if (message.host !== undefined && message.host !== null && message.host !== 0) {
        writer.uint32(tag(1, WIRE.VARINT)).int32(message.host);
    }
    if (message.port !== undefined && message.port !== null && message.port !== "") {
        writer.uint32(tag(2, WIRE.LDELIM)).string(message.port);
    }
}
 ...
 * @returns {Uint8Array}
 */
export function encodeHostPort(message) {
    const writer = Writer.create();
    writeHostPort(message, writer);
    return writer.finish();
}
 ...
 * @returns {HostPort}
 */
function decodeHostPortMessage(reader, length) {
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = {host: 0, port: "" };
    while (reader.pos < end) {
        const tag = reader.uint32();
        switch (tag >>> 3) {
            case 1: {
 ...
 * @returns {HostPort}
 */
export function decodeHostPort(buffer) {
    const reader = Reader.create(new Uint8Array(buffer));
    return decodeHostPortMessage(reader);
}
...
export function writeNetworkingConfig(message, writer) {
    ...
    if (message.ports && message.ports.length > 0) {
        for (const item of message.ports) {
            writer.uint32(tag(2, WIRE.LDELIM)).fork();
            writeContainerPort(item, writer);
            writer.ldelim();
        }
    }
    if (message.hostPorts && message.hostPorts.length > 0) {
        for (const item of message.hostPorts) {
            writer.uint32(tag(3, WIRE.LDELIM)).fork();
            writeHostPort(item, writer);
            writer.ldelim();
        }
    }
}
...
function decodeNetworkingConfigMessage(reader, length) {
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = {mode: 0, ports: [], hostPorts: [] };
    while (reader.pos < end) {
        const tag = reader.uint32();
        switch (tag >>> 3) {
            case 1: {
            ...
            case 2: {
                message.ports.push(decodeContainerPortMessage(reader, reader.uint32()));
                break;
            }
            case 3: {
                message.hostPorts.push(decodeHostPortMessage(reader, reader.uint32()));
                break;
            }
            default:
                reader.skipType(tag & 7);
