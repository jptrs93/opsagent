...
function formEnvVars(form) {
    return Object.fromEntries((form.envVars.val || [])
        .map(v => {
            ...
            if (!key) return null;
            if (v.type === 'secret') return Number(v.secretId || 0) ? [key, {secretRefId: Number(v.secretId)}] : null;
            if (v.type === 'config') return Number(v.configId || 0) ? [key, {configRefId: Number(v.configId)}] : null;
...
function envVarsToFormRows(envVars) {
    return Object.entries(envVars || {})
        ...
        .map(({key, value}) => {
        const secretId = Number(value?.secretRefId || 0);
        const configId = Number(value?.configRefId || 0);
