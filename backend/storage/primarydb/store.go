...
func (s *Storage) FetchDeploymentSnapshot(predicate storage.DeploymentPredicate) []apigen.DeploymentConfig {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) FetchDeletedDeploymentSnapshot(predicate storage.DeploymentPredicate, limit int) []apigen.DeploymentConfig {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) FetchDeploymentConfig(deploymentID int32) *apigen.DeploymentConfig {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) MustFetchDeploymentConfigSnapshotAndSubscribe(predicate storage.DeploymentPredicate) ([]apigen.DeploymentConfig, chan apigen.DeploymentConfig, func()) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) FetchScheduledSnapshotWithLatestFinal(predicate storage.ScheduledInstancePredicate) []apigen.ScheduledInstanceState {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) MustFetchScheduledSnapshotWithLatestFinalAndSubscribe(predicate storage.ScheduledInstancePredicate) ([]apigen.ScheduledInstanceState, chan apigen.ScheduledInstanceState, func()) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) configSnapshotLocked(predicate storage.DeploymentPredicate) []apigen.DeploymentConfig {
...
func (s *Storage) instanceSnapshotWithLatestFinalLocked(predicate storage.ScheduledInstancePredicate) []apigen.ScheduledInstanceState {
...
func (s *Storage) retainFinalizedLocked(state *apigen.ScheduledInstanceState) {
	inst := &state.Instance
...
// load/create paths.
func (s *Storage) configForInstanceLocked(inst *apigen.ScheduledInstance) *apigen.DeploymentConfig {
	if inst == nil {
		return nil
	}
...
func (s *Storage) loadConfigVersionLocked(deploymentID, version int32) *apigen.DeploymentConfig {
...
func (s *Storage) notifyConfigLocked(id int32) {
