...
func scanAssetEvent(row scanner) (apigen.AssetEvent, error) {
	...
	err := row.Scan(&e.EventID, &e.Seq, &e.EventTime, &e.CreatedTime, &e.Author, &e.AssetID, &e.Version, &e.ValueVersion, &e.Value.Fs.Key, &e.Value.Fs.DirectoryID, &e.Value.SpaceID, &e.Value.SizeBytes, &e.Value.Sha256, &e.EventType)
	return e, err
}
...
func (q *Queries) GetLatestAssetEvent(ctx context.Context, id int64) (apigen.AssetEvent, error) {
	return scanAssetEvent(q.db.QueryRowContext(ctx, `SELECT id, global_seq, event_time, created_time, author, asset_id, version,
       value_version, key, asset_directory_id, space_id, size_bytes, sha256, event_type
...
func (q *Queries) ListAssetEventsAtSeq(ctx context.Context, id int64) ([]apigen.AssetEvent, error) {
	rows, err := q.db.QueryContext(ctx, `SELECT id, global_seq, event_time, created_time, author, asset_id, version,
       value_version, key, asset_directory_id, space_id, size_bytes, sha256, event_type FROM asset_event_log WHERE global_seq = ? ORDER BY id`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
