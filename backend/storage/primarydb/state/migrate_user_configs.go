	...
	if err := s.q.Tx(ctx, func(q *pq.Queries) error {
		...
		for _, row := range deployments {
			...
			for _, value := range container.Runtime.EnvVars {
				if value != nil && value.ConfigVersionID != nil && *value.ConfigVersionID > 0 {
					pinned[int64(*value.ConfigVersionID)] = true
				}
			}
		}
		...
		configByOldRowID := make(map[int64]int64)
		...
		for _, row := range deployments {
			...
			for _, value := range container.Runtime.EnvVars {
				...
				oldID := int64(*value.ConfigVersionID)
				seq, ok := seqByOldRowID[oldID]
				if !ok {
					continue
				}
				newID := int32(seq)
				value.ConfigVersionID = &newID
				value.ConfigRef = &apigen.ValueRef{ID: int32(configByOldRowID[oldID]), Version: seq}
				changed = true
			}
			if changed {
