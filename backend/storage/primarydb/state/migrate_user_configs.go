...
func (s *Service) migrateConfigsToEventLog() {
	...
	if err := s.q.Tx(ctx, func(q *pq.Queries) error {
		maxLegacy, err := q.MaxLegacyVersionRowID(ctx)
		if err != nil {
			return fmt.Errorf("max legacy version row id: %w", err)
		}
		if maxLegacy > 0 {
			if err := q.SetEventSeqFloor(ctx, maxLegacy); err != nil {
				return fmt.Errorf("set event seq floor: %w", err)
			}
		}
