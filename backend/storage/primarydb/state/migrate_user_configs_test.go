...
func TestMigrateConfigsToEventLog(t *testing.T) {
	...
	spec.Container1Spec.Runtime.EnvVars = map[string]*apigen.EnvVarValue{
		"LOG_LEVEL": {ConfigVersionID: &pinnedID},
	}
	...
	if env.ConfigVersionID == nil || int64(*env.ConfigVersionID) != env.ConfigRef.Version {
		t.Fatalf("deprecated field %v != ref version %d", env.ConfigVersionID, env.ConfigRef.Version)
	}
	value, ok := s.ResolveConfig(*env.ConfigVersionID)
	if !ok || value != "debug" {
