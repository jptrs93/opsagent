...
func (s *Storage) MustUpsertEnrollmentRequest(requestingIPAddress, requestingMachineID, opendeployVersion, underlayAddress string) *apigen.EnrollmentRequestStatus {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) MustMarkEnrollmentDisconnected(id int32, requestingMachineID string) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) ListEnrollmentRequests() ([]*apigen.EnrollmentRequestStatus, error) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
	return s.listEnrollmentRequestsLocked()
}

func (s *Storage) MustFetchEnrollmentSnapshotAndSubscribe() ([]*apigen.EnrollmentRequestStatus, chan apigen.EnrollmentRequestStatus, func(), error) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) listEnrollmentRequestsLocked() ([]*apigen.EnrollmentRequestStatus, error) {
...
func (s *Storage) AcceptEnrollmentRequest(id int32, workerName, requestingMachineID, underlayAddress string, expectedUpdatedAt time.Time) (*apigen.EnrollmentRequestStatus, error) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
