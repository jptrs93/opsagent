...
func (s *Storage) EnsurePrimaryNode(name, identifier string) *Node {
	s.Mu.Lock()
...
func (s *Storage) MustSetNodeAddresses(id int32, addresses []string) *Node {
	s.Mu.Lock()
...
func (s *Storage) NormalizeNodeUnderlay(identifier, raw string) (string, error) {
...
func (s *Storage) listNodesLocked() []*Node {
...
func (s *Storage) FetchNetworkMapInputs() ([]*Node, []apigen.ScheduledInstanceState) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) ListClusterNodes() []*apigen.ClusterNode {
	nodes := s.ListNodes()
...
func (s *Storage) SubscribeNodeUpdates() (*pubsubu.Sub[apigen.ClusterNode], func()) {
	sub := s.nodeSubs.Subscribe(nil)
	return sub, sub.UnsubscribeFunc
}

func (s *Storage) ListNodeStatuses() []*apigen.ClusterNodeStatus {
	s.Mu.Lock()
	defer s.Mu.Unlock()
	rows, err := s.db.QueryContext(context.Background(), `
...
func (s *Storage) SetNodeStatusByIdentifier(identifier string, connected bool, connectedAt time.Time) {
	s.Mu.Lock()
	ctx := context.Background()
...
func (s *Storage) NodeIdentifierByID(nodeID int32) (string, error) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
...
func (s *Storage) PrimaryNodeID() (int32, error) {
	s.Mu.Lock()
	defer s.Mu.Unlock()
	var nodeID int32
...
func (s *Storage) RenameNode(identifier, name string) (*apigen.ClusterNode, error) {
	s.Mu.Lock()
	node, err := scanNodeRows(s.db.QueryRowContext(context.Background(), `
...
func (s *Storage) SetNodeAllowedSpaces(identifier string, spaces []int32) (*apigen.ClusterNode, error) {
	s.Mu.Lock()
	node, err := scanNodeRows(s.db.QueryRowContext(context.Background(), `
...
func (s *Storage) AllowSpaceOnAllNodes(spaceID int32) {
...
func (s *Storage) RemoveSpaceFromAllNodes(spaceID int32) {
	s.updateAllNodeAllowedSpaces(func(spaces []int32) []int32 {
		out := spaces[:0:0]
		for _, id := range spaces {
			if id != spaceID {
...
func (s *Storage) updateAllNodeAllowedSpaces(fn func([]int32) []int32) {
	s.Mu.Lock()
...
func (s *Storage) SubscribeNodeStatusUpdates() (*pubsubu.Sub[apigen.ClusterNodeStatus], func()) {
