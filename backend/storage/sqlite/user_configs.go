...
func (s *PrimaryStorage) RenameUserConfig(name, newName string) (*apigen.UserConfig, bool) {
	...
	if ok {
		rows, err := s.q.ListUserConfigVersionsByName(context.Background(), newName)
		if err != nil {
			panic(fmt.Sprintf("ListUserConfigVersionsByName after rename: %v", err))
		}
		for _, row := range rows {
			...
			s.userConfigValueSubs.Notify(*renamed)
		}
	}
	return cfg, ok
}
