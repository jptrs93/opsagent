...
import (
	...
	network2 "github.com/jptrs93/opsagent/backend/lib/network"
...
func Run(ctx context.Context, cfg Config) {
	...
	loadCachedClusterNetwork(store)
	loadCachedDataplaneDeploymentID(store, cfg.MachineName)
	...
	preparer2.GHRel = preparer2.NewGithubReleaseDownloader(cfg.DataDir, githubCredentials)

	ctrdClient := ctrd.Connect(ainit.StaticConfig.ContainerdAddress, ainit.StaticConfig.ContainerdNamespace)
	preparer2.NixDocker = preparer2.NewNixDockerBuilder(cfg.DataDir, githubCredentials, ctrdClient)
	preparer2.ContainerImg = preparer2.NewContainerImagePuller(ctrdClient, preparer2.GHRel)
	preparer2.Assets = NewPrimaryAssetProvider(primaryURL, primaryHTTPClient)
	...
	go dataplane.RunNetStateWriter(ctx, store, cfg.MachineName, dataplane.NetStatePath(cfg.DataDir))
...
func dispatchFromPrimary(ctx context.Context, out *outbox, store *sqlite.SecondaryStorage, tracker *logStreamTracker, msg *apigen.MsgToWorker) {
	...
	case msg.ClusterNetwork != nil:
		applyClusterNetwork(store, msg.ClusterNetwork)
	case msg.StopLogRequestID != "":
...
func loadCachedClusterNetwork(store *sqlite.SecondaryStorage) {
	...
	if err != nil {
		slog.Warn("decoding cached cluster network failed", "err", err)
		return
	}
	if err := installClusterNetwork(info); err != nil {
		slog.Warn("installing cached cluster network failed", "err", err)
	}
}

func applyClusterNetwork(store *sqlite.SecondaryStorage, info *apigen.ClusterNetworkInfo) {
	if err := installClusterNetwork(info); err != nil {
...
func installClusterNetwork(info *apigen.ClusterNetworkInfo) error {
	if info == nil {
		return nil
	}
	p, err := network2.ParsePrefix(info.UlaPrefix)
	if err != nil {
		return err
	}
	network2.Default.SetPrefix(p)
	return nil
}
...
func applySnapshot(out *outbox, store *sqlite.SecondaryStorage, snap *apigen.DeploymentWithStatusSnapshot) {
	...
	for _, item := range snap.Items {
		if item != nil {
			installDataplaneDeploymentID(&item.Config)
		}
	}
	for _, item := range snap.Items {
...
func applyConfigUpdate(store *sqlite.SecondaryStorage, cfg *apigen.DeploymentConfig) {
	...
	installDataplaneDeploymentID(cfg)
	slog.Info("applying deployment config update from primary", "id", cfg.ID, "seqNo", cfg.Version)
	store.MustWriteDeploymentConfig(cfg)
}

func loadCachedDataplaneDeploymentID(store *sqlite.SecondaryStorage, machine string) {
	for _, item := range store.FetchDeploymentSnapshot(machine) {
		installDataplaneDeploymentID(&item.Config)
	}
}

func installDataplaneDeploymentID(cfg *apigen.DeploymentConfig) {
	if sqlite.IsDataplaneDeploymentConfig(cfg) && cfg.ID != 0 {
		network2.Default.SetDataplaneDeploymentID(cfg.ID)
	}
}
