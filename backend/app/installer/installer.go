...
func parseInstallPrimary(args []string) (string, installOptions, error) {
	...
	fs.Visit(func(f *flag.Flag) {
		...
		case "web-listen":
			...
			opts.webListen = &v
		case "web-tls-self-managed":
			v, err := strconv.ParseBool(*webTLSSelfManagedRaw)
			if err != nil && parseErr == nil {
				parseErr = fmt.Errorf("invalid --web-tls-self-managed value %q: use true or false", *webTLSSelfManagedRaw)
				return
			}
			opts.webTLSSelfManaged = &v
		case "web-tls-cert-pem-file":
			path := strings.TrimSpace(*webTLSCertPEMFileRaw)
			if path == "" {
			...
			if err != nil && parseErr == nil {
				parseErr = fmt.Errorf("reading --web-tls-cert-pem-file: %w", err)
				return
			}
...
Usage:
  %[1]s install primary [--version vX.Y.Z] [--use-self] [--http-only true] [--web-listen :8080] [--web-tls-self-managed true] [--web-tls-cert-pem-file cert.pem] [--cluster-listen :9443] [--enrollment-listen :9444] [--acme-hosts host1,host2] [--primary-name primary] [--restore-backup true --restore-s3-access-key-id ... --restore-s3-secret-access-key ... --restore-s3-bucket ... --restore-s3-path ... --restore-s3-region ... --recovery-code ...] [--dry-run]
