...
func parseInstallPrimary(args []string) (string, installOptions, error) {
	...
	fs.Visit(func(f *flag.Flag) {
		...
		case "passkey-extra-origins":
			v := strings.TrimSpace(*passkeyExtraOriginsRaw)
			if err := validateEnvValue("--passkey-extra-origins", v); err != nil && parseErr == nil {
				parseErr = err
				return
			}
			opts.passkeyExtraOrigins = &v
		case "cluster-listen":
