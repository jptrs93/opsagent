...
func TestEnsureWebUILocalTLSIssuesUnderLocalCAAndReissuesForNewNames(t *testing.T) {
	...
	bundle, caPEM, err := EnsureWebUILocalTLS(store, certu.WebUITLSNames("opendeploy.local", ":8443"))
	if err != nil {
	...
	_, leaf, err := certu.ParseCertificate(bundle, "leaf")
	if err != nil {
		t.Fatal(err)
	}
	_, ca, err := certu.ParseCertificate(caPEM, "ca")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !webUIBundleCurrent(bundle, caPEM, certu.WebUITLSNames("opendeploy.local", ":8443")) {
		t.Fatal("freshly issued bundle reported not current")
	}
	again, caAgain, err := EnsureWebUILocalTLS(store, certu.WebUITLSNames("opendeploy.local", ":8443"))
	if err != nil {
	...
	reissued, caSame, err := EnsureWebUILocalTLS(store, certu.WebUITLSNames("opendeploy.local,other.local", ":8443"))
	if err != nil {
	...
	_, leaf2, _ := certu.ParseCertificate(reissued, "leaf")
	...
	dir := t.TempDir()
	for range 2 {
		if err := certu.WriteWebUILocalCAFile(dir, caPEM); err != nil {
...
func TestWebUILocalTLSUsesECDSAP256(t *testing.T) {
	store := newTestSecrets(t)
	bundle, caPEM, err := EnsureWebUILocalTLS(store, certu.WebUITLSNames("", ":8443"))
	if err != nil {
		t.Fatalf("EnsureWebUILocalTLS: %v", err)
	}
	for _, c := range []struct {
		...
		_, cert, err := certu.ParseCertificate(c.pem, c.label)
		if err != nil {
			t.Fatal(err)
		}
