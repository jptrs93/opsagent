...
func Initialize(dataDir string, store *state.Service) (*Manager, error) {
	m := newManager(dataDir, store)
	if slots := listKeyslots(m.q); len(slots) != 0 {
...
func (m *Manager) createLocked(name string, value []byte, author, spaceID, directoryID int32) (Meta, error) {
	...
	rec, err := CreateWithVersion(m.store, name, spaceID, directoryID, author, m.sealFuncLocked(value))
	if err != nil {
		return Meta{}, err
	}
	m.cache[rec.ID] = rec
