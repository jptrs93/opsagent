...
import (
	...
	secretspkg "github.com/jptrs93/opsagent/backend/lib/secrets"
...
func TestValidateDeploymentSpecDefaultsHostNetworking(t *testing.T) {
	spec, err := validateDeploymentSpecWithAssets(&apigen.DeploymentSpec{
		Prepare: apigen.PrepareConfig{ContainerImage: &apigen.ContainerImageConfig{Image: "nginx"}},
...
func TestValidateDeploymentSpecAcceptsVirtualHostPorts(t *testing.T) {
	spec, err := validateDeploymentSpecWithAssets(&apigen.DeploymentSpec{
		Prepare: apigen.PrepareConfig{ContainerImage: &apigen.ContainerImageConfig{Image: "nginx"}},
		Runner:  apigen.RunnerConfig{Container: apigen.ContainerRunnerConfig{}},
		Networking: apigen.NetworkingConfig{
			Ports:     []*apigen.ContainerPort{{Name: " http ", Port: 8080}},
			HostPorts: []*apigen.HostPort{{Host: 18080, Port: " http "}},
		},
	}, nil)
	if err != nil {
		t.Fatalf("validateDeploymentSpecWithAssets failed: %v", err)
	}
	if spec.Networking.Ports[0].Name != "http" || spec.Networking.HostPorts[0].Port != "http" {
		t.Fatalf("networking names not normalized: %+v", spec.Networking)
	}
}
...
func TestValidateDeploymentSpecRejectsInvalidNetworking(t *testing.T) {
	tests := []struct {
		...
		{
			name: "host port unknown target",
			spec: apigen.DeploymentSpec{
				Prepare:    apigen.PrepareConfig{ContainerImage: &apigen.ContainerImageConfig{Image: "nginx"}},
				Runner:     apigen.RunnerConfig{Container: apigen.ContainerRunnerConfig{}},
				Networking: apigen.NetworkingConfig{HostPorts: []*apigen.HostPort{{Host: 18080, Port: "http"}}},
			},
			want: "unknown container port",
		},
		{
			name: "host mode with host ports",
			spec: apigen.DeploymentSpec{
				Prepare: apigen.PrepareConfig{ContainerImage: &apigen.ContainerImageConfig{Image: "nginx"}},
				Runner:  apigen.RunnerConfig{Container: apigen.ContainerRunnerConfig{}},
				Networking: apigen.NetworkingConfig{
			...
			want: "require virtual mode",
		},
		{
			name: "host mode rollover",
			spec: apigen.DeploymentSpec{
				Prepare: apigen.PrepareConfig{ContainerImage: &apigen.ContainerImageConfig{Image: "nginx"}},
				Runner: apigen.RunnerConfig{Container: apigen.ContainerRunnerConfig{
...
func TestValidateDeploymentSpecRejectsDataplaneImage(t *testing.T) {
	_, err := validateDeploymentSpecWithAssets(&apigen.DeploymentSpec{
		Prepare: apigen.PrepareConfig{ContainerImage: &apigen.ContainerImageConfig{Image: internaldeploy.DataplaneImage}},
		Runner:  apigen.RunnerConfig{Container: apigen.ContainerRunnerConfig{}},
	}, nil)
	if err == nil || !strings.Contains(err.Error(), "internal-only") {
		t.Fatalf("err = %v, want dataplane image internal-only rejection", err)
	}
}
...
func TestDeploymentDeleteAllowsStaleDisconnectedSystemDeployment(t *testing.T) {
	...
	h := &Handler{
		Store:          store,
		MachineName:    "primary",
		ClusterPrimary: clusterserver.New(store, nil, nil, nil, network.Prefix{}),
	}
