...
import (
	...
	"github.com/jptrs93/opsagent/backend/lib/engine/logfilter"
	"github.com/jptrs93/opsagent/backend/lib/engine/logreader"
...
func (h *Handler) PostV1DeploymentUpdate(ctx apigen.Context, req *apigen.DeploymentUpdateRequest) (*apigen.DesiredState, error) {
	...
	if !req.Spec.IsZero() {
		nextSpec := req.Spec
		if shouldPreserveExistingNetworkingMode(&cfg.Spec, &nextSpec) {
			nextSpec.Networking.Mode = cfg.Spec.Networking.Mode
		}
		validated, err := h.validateDeploymentSpec(&nextSpec)
		if err != nil {
			return nil, err
		}
		spec = validated
...
func (h *Handler) PostV1DeploymentVersions(ctx apigen.Context, req *apigen.DeploymentVersionsRequest) (*apigen.DeploymentVersions, error) {
	...
	case cfg.Spec.Prepare.NixDockerBuild != nil:
		...
		if branch != "" {
			commits, err = versionprovider2.Git.ListCommits(ctx, repo, branch, 25)
			if err != nil {
	...
	case cfg.Spec.Prepare.GithubRelease != nil:
		...
		releases, err := versionprovider2.GHRel.ListReleases(ctx, cfg.Spec.Prepare.GithubRelease.Repo)
		if err != nil {
	...
	case cfg.Spec.Prepare.ContainerImage != nil:
		tags, err := (versionprovider2.ContainerImageVersionProvider{}).ListTags(ctx, cfg.Spec.Prepare.ContainerImage.Image)
		if err != nil {
...
func validateNetworkingConfig(cfg *apigen.NetworkingConfig, runner *apigen.RunnerConfig) error {
	...
	if cfg.Mode == apigen.NetworkingMode_NETWORKING_MODE_UNSPECIFIED {
		if len(cfg.Ports) > 0 || len(cfg.HostPorts) > 0 {
			cfg.Mode = apigen.NetworkingMode_NETWORKING_MODE_VIRTUAL
		} else {
			cfg.Mode = apigen.NetworkingMode_NETWORKING_MODE_HOST
		}
	}
	switch cfg.Mode {
	case apigen.NetworkingMode_NETWORKING_MODE_VIRTUAL:
		return validateVirtualNetworking(cfg)
	case apigen.NetworkingMode_NETWORKING_MODE_HOST:
		if len(cfg.Ports) > 0 || len(cfg.HostPorts) > 0 {
			return invalidConfigErrf("networking: ports and hostPorts require virtual mode")
		}
		if runner != nil && runner.Container.UpgradeStrategy == apigen.ContainerUpgradeStrategy_ROLLOVER {
			return invalidConfigErrf("networking.mode=host does not support runner.container.upgradeStrategy=ROLLOVER")
		}
		return nil
	default:
...
func shouldPreserveExistingNetworkingMode(existing, next *apigen.DeploymentSpec) bool {
	if existing == nil || next == nil {
		return false
	}
	return existing.Networking.Mode != apigen.NetworkingMode_NETWORKING_MODE_UNSPECIFIED &&
		next.Networking.Mode == apigen.NetworkingMode_NETWORKING_MODE_UNSPECIFIED &&
		len(next.Networking.Ports) == 0 &&
		len(next.Networking.HostPorts) == 0
}

func validateVirtualNetworking(cfg *apigen.NetworkingConfig) error {
	...
	for _, p := range cfg.Ports {
		...
		if !validPortName(p.Name) {
			return invalidConfigErrf("networking.ports.name: invalid name %q", p.Name)
		}
		if p.Port < 1 || p.Port > 65535 {
			return invalidConfigErrf("networking.ports[%s].port must be between 1 and 65535", p.Name)
		}
		if _, exists := ports[p.Name]; exists {
			return invalidConfigErrf("networking.ports.name: duplicate name %q", p.Name)
		}
		ports[p.Name] = p.Port
	}
	seenHost := map[int32]bool{}
	for _, hp := range cfg.HostPorts {
		if hp == nil {
			continue
		}
		hp.Port = strings.TrimSpace(hp.Port)
		if hp.Host < 1 || hp.Host > 65535 {
			return invalidConfigErrf("networking.hostPorts.host must be between 1 and 65535")
		}
		if seenHost[hp.Host] {
			return invalidConfigErrf("networking.hostPorts.host: duplicate host port %d", hp.Host)
		}
		seenHost[hp.Host] = true
		if _, ok := ports[hp.Port]; !ok {
			return invalidConfigErrf("networking.hostPorts.port: unknown container port %q", hp.Port)
		}
	}
	return nil
}
...
func validatePrepareConfig(prepare *apigen.PrepareConfig) error {
	...
	if hasContainer {
		...
		if prepare.ContainerImage.Image == internaldeploy.DataplaneImage {
