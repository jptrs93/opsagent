...
func TestMasterPasswordHashRoundTrip(t *testing.T) {
	...
	service, err := InitializeService(state.Open(dbPath), *DefaultConfig(DefaultInitialConfig()))
	if err != nil {
...
func TestVersionedConfigSnapshotsRedactMasterPasswordHash(t *testing.T) {
	...
	service, err := InitializeService(state.Open(dbPath), *DefaultConfig(initial))
	if err != nil {
...
func TestBackupEnabledDefaultsFalseAndCanBeEnabled(t *testing.T) {
	...
	if err := service.UpdateSettings(service.Snapshot().Settings); !errors.Is(err, state.ErrAssetMigrationInProgress) {
...
func TestWebUIDefaultsPreserveExistingHTTPSInstall(t *testing.T) {
	service, err := InitializeService(state.Open(filepath.Join(t.TempDir(), "primary.db")), *DefaultConfig(DefaultInitialConfig()))
	if err != nil {
		t.Fatalf("NewService: %v", err)
	}
	cfg := service.Snapshot()
