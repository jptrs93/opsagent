...
func (r *containerRunner) Promote() error {
	if r.virtualNetwork() {
		...
		if promoteErr := network.Default.Promote(network.Default.CurrentNet(r.deploymentID), cn, stable); promoteErr != nil {
			return promoteErr
		}
		cn.Addr = stable
		if publishErr := r.publishContainerNet(cn, stable); publishErr != nil {
			return publishErr
		}
	}
	r.publish.Store(true)
...
func (r *containerRunner) startReadinessListener(runNumber int32) (*readinessListener, error) {
	...
	if chmodErr := os.Chmod(sockPath, 0o666); chmodErr != nil {
		_ = listener.Close()
		return nil, chmodErr
	}
...
func chownToUser(path, usr string) error {
	...
	if usr != "" && usr != "root" {
		...
		if n, err := strconv.Atoi(name); err == nil {
			uid, gid = n, n
		} else if u, lookupErr := user.Lookup(name); lookupErr == nil {
		...
		if gname != "" {
			if n, err := strconv.Atoi(gname); err == nil {
				gid = n
			} else if g, groupLookupErr := user.LookupGroup(gname); groupLookupErr == nil {
