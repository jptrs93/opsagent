...
func (r *containerRunner) hostPortRules(cn *network.ContainerNet, stable netip.Addr) []network.HostPortRule {
	if cn == nil || len(r.networking.HostPorts) == 0 {
		return nil
	}
	ports := map[string]uint16{}
	for _, p := range r.networking.Ports {
		if p == nil || p.Port < 1 || p.Port > 65535 {
			continue
		}
		ports[p.Name] = uint16(p.Port)
	}
	rules := make([]network.HostPortRule, 0, len(r.networking.HostPorts))
	for _, hp := range r.networking.HostPorts {
		if hp == nil || hp.Host < 1 || hp.Host > 65535 {
			continue
		}
		targetPort, ok := ports[hp.Port]
		if !ok {
			continue
		}
		rules = append(rules, network.HostPortRule{
			HostPort:   uint16(hp.Host),
