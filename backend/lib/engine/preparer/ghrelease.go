...
func (g *GithubReleaseDownloader) runDownload(ctx context.Context, store storage.OperatorStore, dep *apigen.DeploymentConfig, version string) (string, apigen.PreparationStatus) {
	...
	if script := strings.TrimSpace(gh.DownloadScript); script != "" {
		return g.runDownloadScript(ctx, *gh, ownerRepo, version, script, logFile, writeLog)
	}

	assetPath, err := g.downloadReleaseAsset(ctx, gh.Repo, gh.Asset, version, writeLog)
	if err != nil {
		writeLog("ERROR downloading release asset: %v", err)
		return "", apigen.PreparationStatus_FAILED
	}
	writeLog("download complete, artifact: %s", assetPath)
	return assetPath, apigen.PreparationStatus_READY
}

func (g *GithubReleaseDownloader) downloadReleaseAsset(ctx context.Context, repo, assetName, version string, writeLog func(string, ...any)) (string, error) {
	ownerRepo, err := RepoOwnerName(repo)
	if err != nil {
		return "", fmt.Errorf("parsing repo: %w", err)
	}

	writeLog("fetching release %s tag %s", ownerRepo, version)
	release, err := g.fetchReleaseByTag(ctx, ownerRepo, version)
	if err != nil {
	...
	dstDir, err := g.prepareReleaseDir(ownerRepo, version)
	if err != nil {
