...
func NewNixDockerBuilder(dataDir string, provider githubcredentials.Provider, client *ctrd2.Client) *NixDockerBuilder {
...
func (b *NixDockerBuilder) importStream(ctx context.Context, streamPath string, imageRef string, logFile io.Writer) error {
	...
	_, importErr := b.client.Import(ctx, ctrd2.ImageStream{Reader: stdout, Ref: imageRef})
	if importErr != nil {
		if cmd.Process != nil {
			_ = cmd.Process.Kill()
		}
