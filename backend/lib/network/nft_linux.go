			...
			for _, proto := range []byte{unix.IPPROTO_TCP, unix.IPPROTO_UDP} {
				if rule.TargetV4.Is4() {
					c.AddRule(&nftables.Rule{Table: tbl4, Chain: pre4,
						Exprs: dnatExprs(proto, rule.HostPort, rule.TargetV4.AsSlice(), rule.TargetPort, unix.AF_INET)})
				}
				if rule.TargetV6.Is6() {
					c.AddRule(&nftables.Rule{Table: tbl6, Chain: pre6,
						Exprs: dnatExprs(proto, rule.HostPort, rule.TargetV6.AsSlice(), rule.TargetPort, unix.AF_INET6)})
				}
			}
		}
	}
...
// machine itself ("published on the machine's host interfaces"), so
// container-to-container traffic crossing the host is never captured.
