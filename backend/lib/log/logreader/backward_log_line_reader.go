...
func (r *BackwardLogLineReader) readPreviousRecord() (LogLine, error) {
	if r.offset < log.BinaryRecordTrailerLen {
		return LogLine{}, fmt.Errorf("read %s: truncated record trailer", r.path)
	}
	var suffix [log.BinaryRecordTrailerLen]byte
	if err := r.buf.ReadAt(suffix[:], r.offset-log.BinaryRecordTrailerLen); err != nil {
	...
	recordStart := r.offset - log.BinaryRecordTrailerLen - length - log.BinaryRecordLengthLen
	if recordStart < 0 {
		return LogLine{}, fmt.Errorf("read %s: truncated record length %d", r.path, length)
	}
	var prefix [log.BinaryRecordLengthLen]byte
	...
	if err := r.buf.ReadAt(payload, recordStart+log.BinaryRecordLengthLen); err != nil {
		return LogLine{}, fmt.Errorf("read %s: %w", r.path, err)
	}
