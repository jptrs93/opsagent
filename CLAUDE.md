...
- [docs/future-work/deployment-event-log-implementation-plan.md](docs/future-work/deployment-event-log-implementation-plan.md) — Deployment event log design record (built; `deployment_event_log` replaced the three deployment tables): the version(state_path) concept, schema, settled cost/design decisions, delete/CAS semantics, the one-time migration, and the remaining ship-and-strip and follow-up steps.
