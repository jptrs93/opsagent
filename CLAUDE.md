...
- Normal login is passkey-based; username/password login is an opt-in cluster setting (`auth.password_login_enabled`, installer `--password-login true`) for local and evaluation installs where browsers refuse WebAuthn (plain HTTP away from localhost, untrusted certificates). Fresh primary installs generate and print a high-entropy setup password; it can register passkeys or set a password during bootstrap or recovery until rotated.
