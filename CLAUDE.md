...
- API services are defined one per file in `api-contract/*_service.proto` (`api_service.proto` = `ApiServer`, the public HTTP surface; `cluster_service.proto`; `enrollment_service.proto`); models are split per entity into `api-contract/model_<entity>.proto` (data model shapes) and `model_<entity>_operations.proto` (endpoint request/response shapes; an endpoint returning a clean data model shape directly uses the `model_<entity>.proto` definition). Go and JS models are generated by `cleanproto`.
