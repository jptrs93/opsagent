...
- [docs/future-work/event-log-consolidation-implementation-plan.md](docs/future-work/event-log-consolidation-implementation-plan.md) — Planned merge of secrets/configs/assets/network-policies/nodes multi-table shapes into per-entity event logs: explicit value/space sub-versions, byte-for-byte preservation of pinned value version ids and the secret AAD tuple, Go-side uniqueness for node name/identifier, migration and rollout one-way door.
