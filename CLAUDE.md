...
- [docs/future-work/deployment-validation-layers-implementation-plan.md](docs/future-work/deployment-validation-layers-implementation-plan.md) — Single global write lock and two-layer deployment validation (implemented): referenceMu collapsed into state.Service.Mu for every write path, the on-the-fly LiveState view, pre-lock vs in-lock rule assignment, the invariant framing over (existing, updated), and the lock-order rules.
- [docs/future-work/deployment-event-log-implementation-plan.md](docs/future-work/deployment-event-log-implementation-plan.md) — Deployment event log design record (built, rolled out, and stripped; `deployment_event_log` replaced the three deployment tables): the version(state_path) concept, schema, settled cost/design decisions, delete/CAS semantics, and follow-up steps (rename op, undelete, point-in-time reconstruction).
...
- [docs/future-work/wireguard-transport-implementation-plan.md](docs/future-work/wireguard-transport-implementation-plan.md) — Phase-1 plan for replacing ip6tnl node tunnels with WireGuard (shipped and fallback-free; current state in engineering/networking.md): node-local static keys (never in the DB), mTLS-bound pubkey registration via enrollment/cluster stream, netmap distribution, allowed-ips derived from routed prefixes, netaudit surface, and the phase-2 rotation door.
