...
- [docs/future-work/network-policy-implementation-plan.md](docs/future-work/network-policy-implementation-plan.md) — Network policy boundary plan: anti-spoofing, implicit default connectivity rules (same-space, global-space, netproxy DNS, system egress), `allowed_from` override policies on the netmap (deny schema-reserved), nftables chain shape, and milestones.
