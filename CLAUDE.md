...
- [docs/future-work/deployment-event-log-implementation-plan.md](docs/future-work/deployment-event-log-implementation-plan.md) — Plan to replace the three deployment tables with a single append-only event log: the version(state_path) concept, target schema, settled cost/design decisions, migration steps, and the completed naming groundwork.
