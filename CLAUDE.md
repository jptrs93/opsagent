...
- [docs/engineering/api.md](docs/engineering/api.md) — HTTP API design, code generation, and handler flow, including the primary → browser state stream and the `Commit(ctx, preLockValidate, mutate)` write contract: one global seq per commit with authored events and observed statuses in one `CoreUpdate`, `state.Subscribe`, snapshot retention, observed HLC merge, sidecars, and visibility resets. Consult when making API changes or touching the store write path.
...
- [docs/engineering/log-storage.md](docs/engineering/log-storage.md) — Shipped node-local log storage: WAL bucket layout and frame format, the typed shredder (JSON type is the column type, dotted keys, caps), collector and two-pass commit (day deadline, 64MB or producer exit; tally, schema, write), the level ladder (1 batch, 2 roll-up) and parquet schema (dense `f_<key>__<t>` leaves plus typed spill maps), the `log_files`/`log_file_keys`/commit-marker catalog, the maintenance loop (roll-up, reconciliation, 30-day retention, rewrite protocol with grace unlink and a pending-unlink set), and the query engine (typed cross-variant filter semantics, per-file column binding, row-group pruning).
...
- [docs/future-work/containerized-nix-builds.md](docs/future-work/containerized-nix-builds.md) — Shipped design for Nix builds in a one-shot containerd container: pinned `nixos/nix` image with `nixbld` builder users, one Nix store per repository under `/var/lib/opendeploy-nix` with per-repository serialisation, nix2container JSON output ingested by the agent with layer digest verification, egress-only build network, cgroup limits, the accepted per-repository poisoning stance with scheduled and operator store resets, measured store sizes, and Nix removed from node prerequisites.
- [docs/future-work/containerized-nix-builds-implementation-plan.md](docs/future-work/containerized-nix-builds-implementation-plan.md) — Status of the containerized Nix build-out: what landed per phase (prototype results, nix2container ingest with the `v2` cache schema, `nixstore` and `ctrd.RunBuild`, the build network attachment, store lifecycle with size cap and scheduled and operator resets, harness and e2e cases including the hostile-build flake), deviations from the plan, node settings, and open items.
- [docs/future-work/root-agent-and-runtime-deployment.md](docs/future-work/root-agent-and-runtime-deployment.md) — Proposed simplification: the agent runs as root with the ambient capabilities, sudoers drop-in and socket group removed and the trust boundary stated explicitly (children drop to the `opendeploy` uid), plus containerd and runc as the `opendeploy-runtime` per-node system deployment. The deployment half is superseded by the shipped startup reconcile in `lib/runtimebin` (agent compares the runtime symlinks with the release's pin at boot, upgrades, restarts containerd, verifies and rolls back; never downgrades, never blocks boot); the deployment design stays as the fallback if per-node hold-back is ever needed.
...
- [docs/future-work/core-write-update-implementation-plan.md](docs/future-work/core-write-update-implementation-plan.md) — Two-phase move to a fact-only state stream. Phase 1, planned: an opaque `seal_id` per secret value write surfaced on `Secret` and bound as the AEAD associated data in place of `value_version`, with a zero re-seal migration (legacy ids are `v<n>`), and no-op config value writes that return the current event without a row or seq while still repointing stale deployment references. Phase 2, direction only: `CoreWriteUpdate` with create, update, and delete mutations over entity definitions, snapshot as a compacted mutation log, seq-based concurrency tokens, observed statuses and derived data outside the fact message, and restart as a visible diff.
- [docs/future-work/value-reference-pairs-implementation-plan.md](docs/future-work/value-reference-pairs-implementation-plan.md) — Move of secret, config, and asset references (steps 1–4 implemented; e2e run and migration removal open) from event log row ids to `(entity id, value version)` pairs: the reference surface across deployment specs, system settings, ACME bindings, worker caches, and the cluster wire; target `ValueRef` shapes with reserved old tags; partial unique indexes and pair lookups; the one-shot startup blob rewrite of `deployment_event_log` and `system_config_revisions`; the protocol bump and worker cache keys; rollout behaviour and rejected alternatives.
